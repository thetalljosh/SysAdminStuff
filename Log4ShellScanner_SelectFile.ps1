...
function display-logo{
    ...
    write-host "Reviewed and updated by Joshua Lambert, CECOM SEC EISD ITSB Senior Systems Administrator, for use on SEC-LAN." -ForegroundColor Green
...
function Scan-MultipleSystems{
    ...
    get-job | remove-job
    $date = get-date -Format "yyyy-MM-dd_hh-mm-ss"
    write-host "Please select a file with your list of computers."
    $comps = get-openFile
    #$MaxDays = (Get-Date).AddDays(-15)
    #$comps = (Get-ADComputer -Filter {(LastLogonTimeStamp -gt $MaxDays)} -Properties LastLogonTimeStamp, OperatingSystem | Sort-Object Name | Select-Object Name, {[DateTime]::FromFileTime($_.lastLogonTimestamp)}).Name
...
function collect-logs{
    ...
    $creds = Get-Credential -Message "Caution: Script will run even if you do not type your password correctly:"
    foreach ($comp in $comps){
    if(test-path "\\$comp\C$\Log4jSherlock"){
    write-host "Collecting logs from $comp"

    move-item "\\$comp\C$\Log4jSherlock" -Destination "C:\Log4Logs\$Comp\"
...
$files = (get-childitem -path "C:\Log4Logs\" -Recurse -filter *.csv | select-object -ExpandProperty fullname)


foreach($file in $files)
{
if((get-content $file) -like "*true*"){
write-host "Vuln found in $file" -ForegroundColor Red -BackgroundColor Yellow
}
}
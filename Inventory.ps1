    ...
    process {
        foreach ($Computer in $ComputerName) {
            try {
                ...
                if ($socket.Connected) {
                Write-Host "Working on system: $Computer"
...
$Targets = (get-content C:\dev\AllSystems.txt)
Foreach ($PC in $targets){
Get-RemoteProgram -ComputerName $PC -Property DisplayVersion,InstallDate | Format-Table >> C:\temp\Software_Inventory.csv -ErrorAction SilentlyContinue
}